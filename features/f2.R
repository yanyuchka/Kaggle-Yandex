...
###Extract the number of user's unique queries among the all sessions
###To get population: f2.val 
...
extractFeature  <- function(tb.name, f.name) {
  ...
  sqldf(paste("CREATE TABLE ",tb.name, " as 
				SELECT user_id as id,
					COUNT(DISTINCT query_id) as ", f.name, 
				" from 
                                   session S 
...
extractFeature(tb.name ="f2",f.name="val")
